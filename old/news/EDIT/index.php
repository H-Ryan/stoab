		...
		if( strlen($content) > 0 )
		{
			if( $handle = fopen($_GET['where'].$_GET['file'] , 'w') )
			{
				if( is_resource($handle) )
				{
					...
					$warning_message .= '$handle is no resource. is_resource() returned false.';
			}
			else
				$warning_message .= "Invalid file!";
		}
		else
			$warning_message .= "There was no content to save... Press back button and save elsewhere.";
	}
  if($warning_message){
	echo "
...
<div class="wrap">
<h2><a href='http://www.tolktjanst.se/news/EDIT/?do=edit&file=nyheter.txt&where=../../&ext=txt'>Nyheter -- klicka!</a></h2>
...
##Edit below to state if a file type can be edited or not  
    ...
    <tr>
      ...
      <td><?php if($allow_edit == '1') { ?>
        ...
        <?php } else {echo '-';} ?></td>
      <td><a href="?do=delete_file&file=<?=$file?>&where=<?=$where?>" onclick="return confirm('Are you sure?')"></a></td>
		...
		if($size > 0)
		{
			...
			<link rel="stylesheet" href="docs/style.css" type="text/css">
					...
					<script type="text/javascript">
						WYSIWYG.attach('content', full);</script>

			</head><body><center>
			<div class="wrap">
			<h2>Edit File | <?=breadcrumb($_GET['where']);?></h2>
			...
			<form action="?do=save_file&where=<?=$where?>&file=<?=$_GET['file']?>" method="post" enctype="multipart/form-data">
			  <textarea id='content' name='content' style='width:860px;height:500px;'><?=$data?>
			</textarea>
			  <label>
				<input type="submit" name="save_file" id="button" value="Spara ändringar" />
			  </label>
			</form>
	<?php
		}//$size check
		else {
			echo '<p>',$file, ' seems not to have content. Filesize is read to be: ',$size,'</p>';
		}
	}//if is_file
	else {
		echo '<p>Sorry, ',$file,' was not found</p>';
	}
}//edit_file
...
<br />
<br />
<span style="size:14px; border-bottom:dotted #333; 1px;">Support: 0700 23 50 32</span></div>
</body></html>