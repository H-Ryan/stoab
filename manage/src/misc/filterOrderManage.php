            ...
            if (isset($_GET['sortOption'])) {
                if ($_GET['sortOption'] == 0) {
                    ...
                    if (!empty($orderNum) && empty($clientNum)) {
                        ...
                        $statement = $con->prepare("SELECT * FROM t_order WHERE o_orderNumber=:orderNum AND (o_kundNumber=:clientNum AND o_date >= CURRENT_DATE OR ((DATE_ADD(o_date, INTERVAL +1 DAY)) = CURRENT_DATE AND TIMESTAMP(DATE_ADD(o_date, INTERVAL +1 DAY), '08:15:00') > NOW()))");
                        $statement->bindParam(":orderNum", $orderNum);
                        $statement->bindParam(":clientNum", $clientNum);

                    }
                } else if ($_GET['sortOption'] == 1) {

                    if (!empty($orderNum) && empty($clientNum)) {
                        $statement = $con->prepare("SELECT * FROM t_order WHERE o_orderNumber=:orderNum AND (o_date >= CURRENT_DATE OR ((DATE_ADD(o_date, INTERVAL +1 DAY)) = CURRENT_DATE AND TIMESTAMP(DATE_ADD(o_date, INTERVAL +1 DAY), '08:15:00') > NOW()))");
                        $statement->bindParam(":orderNum", $orderNum);
                    } else if(empty($orderNum) && !empty($clientNum)) {
                        $statement = $con->prepare("SELECT * FROM t_order WHERE o_kundNumber=:clientNum AND (o_date >= CURRENT_DATE OR ((DATE_ADD(o_date, INTERVAL +1 DAY)) = CURRENT_DATE AND TIMESTAMP(DATE_ADD(o_date, INTERVAL +1 DAY), '08:15:00') > NOW())) ORDER BY FIELD(o_state,'IC','O','B', 'EC')");
                        $statement->bindParam(":clientNum", $clientNum);
                    } else {
                        $statement = $con->prepare("SELECT * FROM t_order WHERE o_orderNumber=:orderNum AND (o_kundNumber=:clientNum AND o_date >= CURRENT_DATE OR ((DATE_ADD(o_date, INTERVAL +1 DAY)) = CURRENT_DATE AND TIMESTAMP(DATE_ADD(o_date, INTERVAL +1 DAY), '08:15:00') > NOW()))");
                        $statement->bindParam(":orderNum", $orderNum);
                    ...
                    if(isset($_GET['lang'])) {
                        ...
                        if (!empty($orderNum) && empty($clientNum)) {
                            ...
                            $statement = $con->prepare("SELECT * FROM t_order WHERE o_kundNumber=:clientNum AND o_language=:lang AND (o_date >= CURRENT_DATE OR ((DATE_ADD(o_date, INTERVAL +1 DAY)) = CURRENT_DATE AND TIMESTAMP(DATE_ADD(o_date, INTERVAL +1 DAY), '08:15:00') > NOW())) ORDER BY FIELD(o_state,'IC','O','B', 'EC')");
                            $statement->bindParam(":clientNum", $clientNum);
