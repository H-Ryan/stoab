...
if (isset($_POST['orderer']) && isset($_POST['organizationNumber']) && isset($_POST['clientNumber']) && isset($_POST['client'])
    && isset($_POST['language']) && isset($_POST['type']) && isset($_POST['tolk_type']) && isset($_POST['date']) && isset($_POST['start_hour']) && isset($_POST['start_minute'])
        ...
        if ($statement->rowCount() > 0) {
            ...
            if ($statement->rowCount() > 0) {
                ...
                if ($statement->rowCount() > 0) {
                    ...
                    $var = ["{orderNumber}", "{contactPerson}", "{organization}", "{email}",
                        "{telephone}", "{mobile}", "{address}", "{post_code}", "{city}", "{client}",
                        "{language}", "{interpType}", "{date}", "{timeStart}", "{timeEnd}"];
                    $val = [$orderNumber, $contactPerson, $organization, $email, $telephone, $mobile, $address,
                        $post_code, $city, $client, $language, $interpType, $date, $timeStart, $timeEnd];
