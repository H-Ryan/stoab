...
ORDER BY o_date ASC LIMIT :start, 10");
        ...
        if($statement->rowCount() >= 0)
        {
            ...
            $statement = $con->prepare("SELECT COUNT(*) AS id FROM t_order WHERE o_tolkarPersonalNumber =:personal_number
AND o_state=:state AND (o_date >= CURRENT_DATE OR ((DATE_ADD(o_date, INTERVAL +1 DAY)) = CURRENT_DATE
AND TIMESTAMP(DATE_ADD(o_date, INTERVAL +1 DAY), '08:15:00') > NOW()))");
            $statement->bindParam(":personal_number", $personal_number);
            $statement->bindParam(":state", $state);
            $statement->execute();
